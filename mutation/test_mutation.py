...
if __name__ == '__main__':
    ...
    build_ca(issuer_private_key=None, ca_path="./my_repo/ca_certificate.cer", config=config,
             key_export_path=ta_private_key_path)

    subca_private_key_path="./my_repo/key/sub_ca_private_key.pem"
    
    data_tmp = json.load(open("./mutation/data/data/ca_certificate/sub_ca.json", "r"))
    config = certParser(json_data=data_tmp).parser_cacert()

    # 2. Build Sub CA
    if os.path.exists("./my_repo/ca_certificate") is False:
        os.makedirs("./my_repo/ca_certificate")
    ta_private_key = serialization.load_pem_private_key(open(ta_private_key_path, 'rb').read(), password=None, backend=default_backend())
    build_ca(issuer_private_key=ta_private_key, ca_path="./my_repo/ca_certificate/sub_ca.cer", config=config,
             key_export_path=subca_private_key_path, mutator=[])
    ...
    # 5. Build Subca CRL
    if os.path.exists("./my_repo/ca_certificate/sub_ca") is False:
        os.makedirs("./my_repo/ca_certificate/sub_ca")
    crl_path="./my_repo/ca_certificate/sub_ca/revoked.crl"
    crl_config = CRLConfig(version=1, signature=signatureAlgorithmConfig(oid='1.2.840.113549.1.1.11', parameters=None),
                                issuer="subca", this_update="20241125055723Z", next_update="20301125055723Z",
                                crl_number=CrlNumConfig(0, False), aki_critical=False, revoked_certificates=None)
    build_crl(issuer_private_key_path=subca_private_key_path,
                crl_path=crl_path, config=crl_config)
    
    # 6. Build Subca ROA
    issuer_rsa_key_path = "./my_repo/key/sub_ca_private_key.pem"
    roa_path = "./my_repo/ca_certificate/sub_ca/test_roa.roa"
    roa_config = ROAConfig()
    roa_config.ee_config =eeCertParser(json_data=json.load(open("./mutation/data/data/ca_certificate/sub_ca/roa.json", "r"))["content"]["certificates"][0]).parse_eecert()
    ...
    # 7. Build Subca Manifest
    file_names = [crl_path, roa_path]
    mft_config = MFTConfig()
    mft_config.file_names = file_names
    ee_config = eeCertParser(
        json_data=json.load(open("./mutation/data/data/ca_certificate/sub_ca/manifest.json", "r"))["content"]["certificates"][0]).parse_eecert()

    mft_config.ee_config = ee_config
    mft_path = "./my_repo/ca_certificate/sub_ca/manifest.mft"
    build_mft(issuer_private_key_path=subca_private_key_path,
                mft_config=mft_config,
                mft_path=mft_path)
