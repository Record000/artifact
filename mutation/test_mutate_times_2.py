...
def record_dep_time(field_name, start_time):
    elapsed = time.perf_counter() - start_time
    FIELD_STATS[field_name].append(elapsed * 1_000_000)
...
def build_ca(issuer_private_key, ca_path, config, key_export_path, is_ta=False):
    if not is_ta:
        ...
        # Log.info(f"[MUTATE VALUE]: {target}")
    ...
    if not is_ta:
        ...
        # Log.info(f"Certificate {ca_path} RFC Status: {'VALID' if is_valid_rfc else 'INVALID'}")
...
def build_topology(depth, branching_factor, ta_template, sub_template, mft_template, roa_template, tmp_dir):
    ...
    def mutate_ca_config(node, parent=None):
        ...
        data["tbs_certificate"]["issuer"]["common_name"] = (parent.name if parent else node.name)
        record_dep_time("CA.Issuer.Sync", t)
...
def generate_repository_task(repo_id, tree_depth, branching_factor, base_root, tmp_root, templates_data):
    ...
    def process_node(level, parent_node=None):
        ...
        record_dep_time("CA.Issuer.Sync", t_start)
        ...
        build_ca(parent_priv, node.cert_path, ca_cfg_obj, node.priv_key_path, (level == 0))

        if level == 0:
            export_tal(node.cert_path, os.path.join(repo_dir, "tal/ta.tal"))
        ...
        build_crl(node.priv_key_path, crl_path, minimal_crl_config(node.name))

        mft_files = [crl_path] + [c.cert_path for c in node.children]
        
        if not node.children: 
            t_start = time.perf_counter()
            roa_ee_json = json.loads(json.dumps(templates_data["roa"]))
            ee_cfg_raw = roa_ee_json["content"]["certificates"][0]

            ee_cfg_raw["tbs_certificate"]["issuer"]["common_name"] = node.name
            record_dep_time("EE.Issuer.Sync", t_start)
            
            roa_path = os.path.join(node.physical_dir, "test_roa.roa")
            roa_cfg = ROAConfig()
            roa_cfg.ee_config = eeCertParser(json_data=ee_cfg_raw).parse_eecert()
            build_roa(node.priv_key_path, roa_cfg, roa_path)
            mft_files.append(roa_path)

        t_start = time.perf_counter()
        mft_ee_json = json.loads(json.dumps(templates_data["mft"]))
        mft_cfg = MFTConfig()
        mft_cfg.file_names = mft_files 
        record_dep_time("MFT.Hash.Calc", t_start)
        
        mft_cfg.ee_config = eeCertParser(json_data=mft_ee_json["content"]["certificates"][0]).parse_eecert()
        build_mft(node.priv_key_path, mft_cfg, os.path.join(node.physical_dir, "manifest.mft"))
...
if __name__ == "__main__":
    ...
    print(f"Testing Dependent Field Mutation/Repair Times (Total Repos: {REPOS})...")
    
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        futures = [executor.submit(generate_repository_task, i, 2, 2, BASE_ROOT, TMP_ROOT, cached_templates) 
                   for i in range(REPOS)]
        for f in as_completed(futures):
            f.result()

    results = []
    for field, times in FIELD_STATS.items():
        avg_us = statistics.mean(times)
        semantic = DEP_SEMANTIC_MAP.get(field, "sync other fields")
        results.append((field, semantic, avg_us))

    results.sort(key=lambda x: x[2], reverse=True)

    print("\n| Field Name | Semantic Meaning  | average cost (μs) |")
    print("| :-------------------- | :--------------------------- | :---------------- |")
    for field, semantic, avg in results[:10]:
        print(f"| **{field:<20}** | {semantic:<28} | {avg:<12.4f} μs |")