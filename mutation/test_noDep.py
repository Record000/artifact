...
def check_ski_compliance(tbs, ext_dict):
    ...
    try:
        ...
        # 3.  (MUST be 160-bit SHA-1 hash of SubjectPublicKey)
        spki = tbs['subjectPublicKeyInfo']
        ...
        # 4. length check (160-bit = 20 bytes)
...
def build_ca(issuer_private_key, ca_path, config, key_export_path, is_ta=False):
    if not is_ta:
        ...
        target += dependenciy_targets

        Log.error(f"[MUTATE VALUE]: {target}")
        mutator = [CertMutator(t) for t in target]
    else:#"subjectPublicKeyInfo",
        ta_mutation_targets = ["subject", "subjectPublicKeyInfo", "key_identifier"]
        Log.error(f"[MUTATE TA]: {ta_mutation_targets}")
        mutator = [CertMutator(t) for t in ta_mutation_targets]
...
def export_tal(ca_cert_path, tal_export_path):
    try:
        ...
        Log.success(f"TAL successfully exported to {tal_export_path}")
        return True

    except Exception as e:
        Log.error(f"TAL Export Critical Failure: {e}")
        return False
        ...
        for ext in ee["tbs_certificate"]["extensions"]:
            ...
            if ext["extn_id"] == "subject_information_access":
                for acc in ext["extn_value"]:
                    if acc["access_method"] == "1.3.6.1.5.5.7.48.11": # id-ad-signedObject
...
def build_topology(depth, branching_factor, ta_template, sub_template, mft_template, roa_template, tmp_dir):
    ...
    root.name = generate_mutated_ca_name()
    all_nodes = [root]
    
    def configure_ca_node(node, parent_node=None):
        ...
        for ext in data["tbs_certificate"]["extensions"]:
            ...
            if parent_node and ext["extn_id"] == "authority_information_access":
                parent_cert_url = (
                    f"{parent_uri}/sub_ca.cer" if parent_node.level > 0 
    ...
    def create_children(p_node):
        ...
        for _ in range(branching_factor):
            ...
            child.name = generate_mutated_ca_name()
            p_node.children.append(child)
            configure_ca_node(child, p_node)
            all_nodes.append(child)
            create_children(child)

    configure_ca_node(root)
    ...
    for node in all_nodes:
        ...
        sync_ee_json(node.mft_ee_json, node.name, node.ski, 
                     f"{node.uri}/revoked.crl", f"{node.uri}/manifest.mft")
        
        if not node.children: 
            node.roa_ee_json = os.path.join(tmp_dir, f"roa_L{node.level}_{node.name}.json")
            shutil.copy(roa_template, node.roa_ee_json)
            sync_ee_json(node.roa_ee_json, node.name, node.ski, 
                         f"{node.uri}/revoked.crl", f"{node.uri}/test_roa.roa")
